...
**✅ 全部欄位已對完整官方文件確認**(2026-07-31,爬下整份 docs.payuni.com.tw 後逐項比對,**零推測**):

| 我方實作 | 官方規格 | 結果 |
|---|---|---|
| UPP `Version = 2.0` | 「固定 2.0」 | ✅ |
| `UseTokenType=3` | 1=約定(消費者可自行取消) / 2=記憶卡號 / **3=強制約定(消費者無法取消)** | ✅ |
| `CreditToken` ≤200、`[A-Za-z0-9@.#$%_-+]` | 完全相同,且「如使用 UseTokenType,此為必填」 | ✅ |
| `CreditTokenType=2` | 1=會員(旗下所有商店共用) / **2=商店(僅首次交易商店可用)** | ✅ |
| UPP 回傳 `CreditHash` | 「信用卡Token Hash;**有 CreditToken 且授權成功才會壓碼**」 | ✅ |
| UPP 回傳 `CreditLife` / `Card4No` | 「Token 有效日期 MMYY」/「卡號後四碼」 | ✅ |
| `/api/credit` `Version = 1.3` | 「固定 1.3」 | ✅ |
| `TradeStatus` 1/2/3/**8** | 1=已付款 2=付款失敗 3=付款取消 **8=訂單待確認** | ✅ 我方把 8 歸為「結果未定」正確 |
| `credit_bind/cancel` `Version=1.0` + `UseTokenType`(1/2) + `BindVal` | 完全相同;「當 UseTokenType 為綁定時請帶 **CreditHash**;為記憶卡號時請帶 **CreditToken**」 | ✅ |
| 四支端點 URL | `/api/upp`、`/api/credit`、`/api/credit_bind/query`、`/api/credit_bind/cancel` | ✅ |

**→ P2 原本「只靠文件截圖記憶、沒對真交易驗過」的那三個欄位,現在有官方文件白紙黑字。程式面零未知。**

原始摘要(2026-07-29 截圖確認):
...
**✅ 官方明文（2026-07-31 從完整 API 文件確認,不再是推論）**:

> 「信用卡幕後Token交易(CREDIT)」文件的串接前置作業:
> - 「須向PAYUNi提出申請,**審核開通且綁定幕後授權IP即可使用**。」
> - 「下載申請表單向PAYUNi提出**信用卡Token機制申請與綁定IP,審核通過後即可串接**。」

寫的是「**即可串接**」不是「即可上線」→ **要串接(含測試環境)就得先申請**,沒有測試豁免。
所有「幕後」類 API（信用卡Token、續期收款、超商代碼、街口、AFTEE、免跳轉元件）文件都是同一句話。

**結論**:程式（P1–P5）不受阻,但**任何實刷驗證都要先拿到 07 這張**。原文寫的「不需等 PAYUNi、正式上線前才申請」已更正。
...
> ⚠️ **2026-07-31 重大更正**。本節原本根據「後台表單長相」推論,寫錯了三件事:
> ① 說「每期金額固定」——**錯**,`續期收款訂單內容修改` API 可改每期金額(1~199999)。
> ② 說「期數上限 12 期」——**錯**,官方上限是 **900 期**(後台表單的 12 只是預設值)。
> ③ 說「只能後台建連結、不是 per-customer API」——**錯**,有完整 5 支 API:
>    建立 / 狀態修改 / 訂單內容修改 / **卡號修改** / 訂單查詢。
> 教訓:**看表單推論規格會出錯,要翻 API 文件。**

| | **續期收款** | **信用卡 Token 幕後扣款**(我們實作的) |
|---|---|---|
| 誰發動扣款 | **PAYUNi 按自己的排程扣** | **我方排程主動扣** |
| 每期金額 | 可改(需在扣款日前呼叫「訂單內容修改」) | 每期發動時當下決定 |
| 期數 | 上限 900 期 | 無上限 |
| 建立方式 | API `續期收款建立`(也可後台建連結) | UPP 首刷建約定 |
| 換卡 | ✅ **有「卡號修改」API,不需重新付款** | ❌ 必須重跑一次 UPP 首刷 = **伴隨一期付款** |
| 停止扣款 | 呼叫「狀態修改」終止;**沒終止成功就會繼續扣** | `autoRenew=false` 即可,沒有任何排程會扣 |
| 失敗方向 | 我方降級了、金流仍按排程扣 → 「服務停了卡還在扣」 | 排程沒跑 = 這期沒扣到(頂多少收錢) |
| **申請與綁 IP** | **同樣需要**(見 §12 末) | 同樣需要 |

### 為什麼仍然**不**改用它

1. **換過去不會解掉任何阻塞**（決定性理由）。官方文件白紙黑字:「續期收款幕後機制…**會員須向
   PAYUNi 提出申請,審核開通且綁定幕後授權 IP 即可使用**」。跟 Token 是同一道門檻,
   改架構換不到「現在就能測」。
2. **失敗方向相反**:金流端排程 = 我方降級後若「狀態修改」沒成功,客戶的卡會繼續被扣。
   那正是藍新時代最貴的那種 bug,而我們 2026-07-30 才剛把那套風險結構移除。
3. **折抵/降級的時序脆弱**:要在 PAYUNi 的扣款日**之前**搶著把金額改好;改失敗或晚了,
   就是按舊金額扣。Token 模型是我方發動,金額在發動當下算,沒有時序問題。
4. **P1–P5 已全部完成並實測**(單次付款真刷、發票開立/作廢/折讓、續扣成功/失敗、降級、取消)。
   換架構等於全部重做。

### 但它有一個我們沒有的優點,記著

**「續期收款卡號修改」API 可以在不收費的情況下換卡。** 我方 Token 模型的換卡必須重跑
UPP 首刷 → **一定伴隨一期付款**(見 §8.4①)。若日後客訴集中在這點,可考慮:
用 `credit_bind` 系列查有沒有「純綁卡不收費」的做法,或評估混合架構。
