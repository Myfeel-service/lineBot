...
| 🔴 「轉給真人」混了三種事、自己搞定率被低估 | 2026-08-10 拆帳實測（MYFEEL 八月）：轉真人 90 次＝**補知識有救 62**（no_grounding 51＋product_mismatch 11）＋**要接系統/刻意轉人 15**（order_status 14＋sensitive_topic 1）＋**AI 根本沒出手 13**（客人自己打「找真人」被攔截）。①那 13 次跟 AI 表現無關卻被算進成績單 → 自己搞定率 39% 應為 42% ②「先問清楚 33」是中間步驟不是結局，反問後客人點選再答成功的那次走 followup、**只記 token 不記次數**，所以那 33 次的成功完全看不到（若全算成功上限是 59%）→ 39% 同時被稀釋又被低估。**做法（2026-08-10 重想後定案）**：⛔不拆分段長條（三段相加=100% 是它的優點；事件表資料不全的月份子段會加不回總數）⛔不無聲換分母（同頁兩種分母＝自己製造數字對不起來）。分四步：①記帳補兩顆**子計數器**（沿用 import⊆input 的子集慣例，invocations 恆等式不動、列表與 KPI 一致的約束不破）：`directHandoffs`＝「找真人」捷徑（AI 沒出手，含傳圖後找真人）、`followupAnswered`＝先問清楚後答成功 ②UI 母數改「AI 出手過」= invocations−directHandoffs（子計數 0 時自然退回現狀→舊月份數字不跳動），「客人直接要找人」單獨一行 ③「轉給真人」圖例下加一行原因拆解（事件表按月聚合成文字，不畫進長條；事件不全的月份如實標註） ④「先問清楚」tooltip 標明是中間步驟；等 followupAnswered 有資料再顯示「反問後答成功 N 次」。①②④一起做，③可後補 |
