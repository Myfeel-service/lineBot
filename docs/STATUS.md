...
| AI 表現頁：資訊架構與文案再評估 7 項 | 報告 `docs/AI-PERFORMANCE-PAGE-REDESIGN-20260810.md`。🔴 額度警示寫「本月」但額度是續約日制（同一張卡自己打自己）、副標兩句擠一句且用「則」解釋「則」、「核心指標」是內部術語、趨勢圖一個月也照畫（一個點不是趨勢）、方案額度卡排在所有表現資料之前（先講錢再講價值）、側欄「AI 表現」應移到第一個（日常動線的起點）、Hero 下方補一句由真實數字推出的解讀（價值最高） |
