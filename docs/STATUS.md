...
| 用量監控頁升格成「AI 總覽」 | 它實際上已經是 dashboard（狀態／產出／趨勢／待辦／額度都有），但升格前要解三件事：①**權限錯配**——它是 `usage.read`(admin)，其他 AI 頁是 `ai.read`(viewer)，升格會變成「最該先看的頁權限最嚴、第一線客服看不到」②金額已移走，只剩「方案額度」卡跟計費有關，搬到帳單頁就能降權限 ③頁名「用量/監控」像計量表，建議改「AI 表現」。另建議 Hero 主角從「處理幾則」換成「自己搞定率」（agent 產品賣的是擋掉多少，不是用了多少） |
