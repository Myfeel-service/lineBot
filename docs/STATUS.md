...
| 項目 | 說明 |
|---|---|
| **「自己搞定」要不要含「問完答成功」** | 目前分子只認「直接答完」，反問後客人選完、AI 成功答出的**沒算進成績**（只記在 `followupAnswered` 子計數）→ **現在的自己搞定率被低估**。八月若 33 次反問有 20 次答成，真實約 49% 而非 39%。改＝動成績算法，且 `followupAnswered` 2026-08-10 才開始記、八月只有半個月，建議九月資料滿一個月再拍板。另需一併決定「反問後沒下文」算什麼 |
