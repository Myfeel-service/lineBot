...
>
> **2026-08-14 拍板補註**：§6 三題老闆全數拍板——①分期順序照 0→1→2 ②高風險紅線
> （群發／對客人說話／刪除／憑證／成員／計費，最後一顆按鈕永遠留給人）認可為**永久原則，
> 工程不自行放寬** ③Phase 2 中風險首批**含 AI 設定**。Phase 0 同日動工完成。
> 另更正一處事實：§2.4／§3.4 說 `ai/settings` PUT 是「整份覆寫」**有誤**——實作時查證
> `setAiSettings` 本來就是「先讀現值＋深合併 partial」（`ai-settings.ts:200`），單欄 patch 安全；
> §3.4 鐵律第 5 條的防護由既有實作天然滿足，agent 端不需另做 read-modify-write。
