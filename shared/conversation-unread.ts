...
/**
 * 未讀數字要怎麼寫給人看：`18` 還是 `18＋`。
 *
 * 為什麼需要這個（2026-09-22 老闆回報）：清單是往下捲才載下一頁的，而上面那條篩選
 * **只掃得到已經載進來的那幾頁**，所以這個數字的真正意思是「掃到現在為止找到幾筆」，
 * 往下捲就會變大、按「重整」（清單重頭載）又會變小。
 *
 * 但它被放在一顆按鈕的括號裡，那個位置任何人都會讀成「我總共有幾筆沒看」——
 * 一個會自己長大又會縮回去的總數，看兩次就沒有人再相信它。加上「＋」之後那句話
 * 才變成「至少 18 筆，下面還沒找完」，18＋ 變 23＋ 就不再是矛盾。
 *
 * `allLoaded` ＝清單真的載到底了（後端沒有下一頁、也沒有因為上限被截掉）。
 * 只有這時候括號裡的數字才真的是總數，＋才拿得掉。
 *
 * ⛔ 不是每個地方都該加＋，**會加是因為那個數字在回答「還有多少沒看」**：
 *    「標記全部已讀（N）」的 N 回答的是另一個問題——「這次按下去會清掉幾顆」，
 *    那個範圍就是已載入的那些，是**確定的**。在那顆按鈕上加＋會變成
 *    「按下去不知道會清掉多少」，比沒加還可怕。
 */
export function unreadCountLabel(count: number, allLoaded: boolean): string {
  return allLoaded ? String(count) : `${count}＋`
}
