 ...
 *
 * 2026-08-27 從 `server/utils/` 搬到 `shared/`：前端也要算「昨天是哪一天」
 * （小幫手的昨日摘要）。原本前端自己用 `new Date()` 的本機時區算，人在國外開後台時
 * 「昨天」會跟台灣同事差一天——前後端各一把尺就是這樣長出來的。⛔別再抄第二份，
 * 前端從 `~~/shared/taipei-day` 匯入這一份。
 */
