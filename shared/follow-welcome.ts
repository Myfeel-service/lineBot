...
/**
 * 回覆文字裡可以直接用、**不需要先收集**的變數（`D-23`D，2026-09-22）。
 *
 * ⛔ 以前「插入變數」那顆選單是 `v-if="收集欄位.length"`——沒有收集步驟就整顆不出現，
 *    於是**最常用的「客人的名字」沒有任何地方教得到**（加好友歡迎正是沒有收集步驟的那種）。
 */
export const BUILTIN_SCRIPT_VARIABLES = [
  { key: 'displayName', label: '客人的 LINE 名稱' },
] as const

/**
 * ⚠️ 取不到值時會變成**空字串**（`renderWithAttributes` 對不認得的變數回 `''`），
 * 所以句子要寫成「少了它也讀得通」的形狀。⛔ 這句話一定要講：
 * 不講的話會有人寫「嗨 {{displayName}}！」，取不到名字的客人收到的是「嗨 ！」。
 */
export const BUILTIN_VARIABLE_HINT
  = '「客人的 LINE 名稱」取不到時會變成空白（客人改過隱私設定就可能取不到），'
    + '所以請寫成少了名字也讀得通的句子，例如「{{displayName}} 您好」。'
