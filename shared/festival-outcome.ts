/**
 * 「上一檔做得怎麼樣」——檔期成效回顧（`C-55`②，`C-240`）。
 *
 * ⚠️ **本案原本取號 `C-237`，與另一個 session 同日撞號**（他們 10:15 先入表＝
 *   「另外五頁也吃 `?id=`」），照慣例讓號改為 `C-240`。
 *   ⛔ commit `8cdd45c` 的訊息裡寫的還是 `C-237`，那是撞號前送出的，⛔ 不要據它反推編號。
 *
 * ── 這支在補的洞 ───────────────────────────────────────────────
