...
export interface ConversationDoc {
  ...
  /**
   * 真人最近一次表明「這位客人我在處理」的時間：客服送出訊息（收件匣手打／客服預存）
   * 或按下「我接手」。**有值＝這位客人是真人的，AI／自動回覆不接手**；
   * 真人按「結束會話」或「交還機器人」時清成不存在（見 conversation-session.ts）。
   *
   * 為什麼記在對話上、不是記在會話上：真人常常在**沒有進行中會話**時講話（客服回完按了
   * 「結束會話」、或上一場早就到期被收掉），那則訊息掛不到任何一場，而客人回的下一句
   * 會開新的一場——沒有這個欄位，新的一場只知道「沒人接手」，AI 就把真人的話接走了。
   *
   * ⛔ 刻意**不設時限**（老闆 2026-08-20 拍板：真人沒有自己切，就不要把對話轉回 AI）。
   * 2026-08-20 起才寫，舊對話沒有這欄＝行為與從前完全相同。
   */
  lastHumanActionAt?: Timestamp | null
