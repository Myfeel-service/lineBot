 ...
 *
 * `idle_bot_auto`＝機器人那半的收尾（`D-64`）：客人問完就沒再回來、超過
 * `BOT_SESSION_MAX_IDLE_DAYS` 天沒動靜。⛔ 跟 `idle_auto` **分開記**：一個是
 * 「真人接手了但忘了按結束」（客服要檢討的），一個是「客人自己不回了」（誰都沒錯），
 * 混成同一種原因的話，之後要查「客服漏掉幾場」會查出一堆假數字。
 */
export type SessionCloseReason = 'idle_auto' | 'idle_bot_auto'

/**
 * 機器人在處理（或還沒有人接手）的對話，超過這麼多天沒動靜就自動收尾（`D-64`，2026-09-04 拍板）。
 *
 * **為什麼要有這條**：24 小時換場是**懶惰觸發**——只在「客人下一則訊息進來」那個交易裡
 * 才把上一場關掉。客人問完就沒再回來的話，那場對話永遠停在進行中。
 * 而 AI 讀對話貼標籤**只讀已結束的對話**，所以那些對話 AI 永遠讀不到：
 * 2026-09-04 實測 MYFEEL，8/23 以後有動靜的 770 場裡只有 205 場（26.6%）進得了掃描器，
 * 另有 2,978 場超過 30 天沒動靜卻還掛著。真人接手的對話本來就有收殮機制
 * （`autoCloseIdleHumanSessions`），機器人的那半一直沒有——這是漏掉的一塊，不是刻意的設計。
 *
 * **為什麼是 7 天**：客人隔一兩天回來問後續是常事，那該算同一場；隔一週以上再來，
 * 本來就該算新的一場。
 *
 * ⛔ **不做成可關閉的設定**（跟真人那邊的 `humanSessionMaxIdleHours` 不同）：關掉它就等於
 * 把「AI 只讀得到四分之一的對話」這個問題養回來，而且背景查詢會一直掃這批殭屍場
 * （2026-08-11 讀取費暴衝有這一份）。要調整請改這個常數。
 */
export const BOT_SESSION_MAX_IDLE_DAYS = 7
