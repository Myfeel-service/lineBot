...
export interface AiSettingsDoc {
  ...
  handoffNotify: {
    ...
    /**
     * 每日摘要（昨天的成績＋客人正在等的事＋知識庫待辦）發送時段：台北時間整點 0–23，
     * 當天過了這個小時的第一輪排程發送。**每天都會發**（2026-09-17 `D-81` 拍板），
     * 順利的日子發一行短版——原本「沒事就不發」會讓「昨天很順」與「通知壞了」長得一樣。
     */
    digestHour: number
    /**
...
/**
 * 「內容較短」**提醒**門檻（去空白後的 content 字數）。
 *
 * ⛔ **這不是「不採用」的門檻。** 短卡照樣建立、照樣 embedding、照樣被檢索到
 * （`searchSimilarChunks` 只篩 workspaceId + `status='indexed'`，沒有任何字數條件），
 * 也照樣拿去回答客人。這把尺只決定畫面上標不標一行灰字，不影響任何行為。
 * 畫面文案一度寫成「補到 30 字以上 AI 才找得到」＝假的，照著做的人會另外再建一張
 * 長版的講同一件事（兩張重複卡）。
 *
 * 為什麼是提醒、不是待辦：切壞或抓壞的殘片多半很短，值得有人看一眼；但
 * 「免運門檻：單筆滿 1000 元」這種最好答的知識也才 13 字，催人補長或停用等於
 * 把正常的知識關掉（2026-09-17 老闆拍板：太短就當提醒，不用太強迫）。
 *
 * ⚠️ 它只量 content 一欄，真正進向量的是產品名＋標題＋常見問法＋content 四段
 * （`buildEmbeddingText`）——所以「被標較短」跟「檢索效果差」不能劃等號。
 *
 * **前後端共用同一把尺**：來源頁的逐卡標示與知識庫體檢的計數若各用各的門檻，
 * 會出現「體檢說有 5 張、點進去一張都沒標記」的自相矛盾。
 */
