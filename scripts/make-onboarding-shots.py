...
# 推近的裁切以**框為中心**（老闆 09-11：「盡量讓需要操作的物件在圖片中心，在邊緣的另當別論」）。
# `ZOOM_PAD_*` 只用來保證裁切至少裝得下「框＋左右文脈」；`ZOOM_MIN_LEFT` 是左邊的保底，
# 因為這兩個後台都是「欄位名稱在左、要按的東西在右」，左邊裁掉就不知道在設哪一欄。
...
def build_carousel(prefix: str, spec, x0: int = 240, x1: int = 1330, vh: int = VH) -> None:
    ...
    def zoom_crop(entry, box, t: float):
        ...
        # ⚠️ spec 第 6 格（選填）＝**這一步非看到不可的範圍**，通常是「框所在的那一列」。
        #    只給少數幾格用：小控制項落在長列的**最右端**時，置中就會只剩一顆鈕浮在空白裡
        #    （實測「按儲存」那格：一顆綠鈕配幾條沒有名字的灰線，看不出在存什麼）。
        #    ⛔ 這是**例外不是預設**——多數格置中就對了，別整批塞。
        ctx = entry[5] if len(entry) > 5 else None
        if ctx:
            need_w = max(need_w, (ctx[2] - ctx[0]) + 60)
        k = min(1.0, need_w / width)
        cw, ch = width * k, vh * k
        # 2026-09-11 老闆：「放大時可以盡量讓需要操作的物件在圖片中心嗎，在邊緣的另當別論」
        # ——所以**以框為中心**，靠近畫面邊緣時由下面的夾限自然退讓（那就是「另當別論」）。
        # ⚠️ 但保底要留 `ZOOM_MIN_LEFT` 的左邊文脈：這兩個後台的版面都是「欄位名稱在左、
        #    要按的東西在右」，左邊被裁掉就不知道自己在設哪一欄（2.1 倍那一版實測
        #    「Webhook網址」被裁成「ebhook網址」）。1.75 倍時置中本來就留得夠，
        #    這條只在有人日後把倍率調大時才會咬住。
        cx1 = (box[0] + box[2]) / 2 - cw / 2
        cx1 = min(cx1, box[0] - ZOOM_MIN_LEFT)
        if ctx:   # 非看到不可的範圍優先於置中（上面那條註解說的例外）
            cx1 = max(min(cx1, ctx[0] - 20), ctx[2] + 20 - cw)
...
def main() -> None:
    ...
    # ⚠️ 2026-09-11 再多一格「聊天」（跟輪播那支同一個理由，見下面 `oam-response-settings`）：
    #    這支是**冷啟動**用的（從我們自己的設定頁按「教我怎麼關」進來），那些人更沒看過這一頁。
    build_anim('oam-auto-reply.webp', [
        (oam, 0, None, 900),
        (oam, 0, (1264, 53, 1344, 94), 1500),   # ①右上「設定」
        (oam, 0, (44, 213, 190, 247), 1500),    # ②側欄「回應設定」
        (oam, 0, (548, 276, 600, 312), 2000),   # ③確認「聊天」是開的
    ...
    build_carousel('oam-webhook-url', [
        ...
        # ⚠️ 第 6 格是**推近時非看到不可的範圍**＝「Webhook網址」那一整列（含左邊的欄位名稱）。
        #    這顆「儲存」是小控制項又落在長列的最右端，置中推近之後畫面只剩一顆綠鈕
        #    配幾條沒有名字的灰線，看不出在存什麼。⛔ 這是例外，其餘各格照舊置中。
        (oam_api, 0, (928, 410, 996, 450), 2000, 0, (300, 396, 1000, 464)),
    ], x0=0, x1=1352)

    # ⚠️ 2026-09-11 補第②格「打開**聊天**」（老闆實機截圖抓到）：原本只教 Webhook，
    #    但「回應功能」那一區有**三個開關**，「聊天」就在 Webhook 正上方——關著的話，
    #    LINE 後台的「聊天」頁會整頁變成「『聊天』功能目前關閉中」。
    #    ⛔ 中間那顆「加入好友的歡迎訊息」**刻意不框也不提**（要不要開是他家的事），
    #       所以不能把框放大成一個涵蓋三顆的大框，必須自己一格。
    #    ⚠️ 座標是量出來的不是推出來的：三顆開關的 LINE 綠色塊分別在 y 284–305／460–480／
    #       626–646，取跟 Webhook 那格相同的 8px 外擴。
    build_carousel('oam-response-settings', [
        (oam, 0, None, 900),
        (oam, 0, (44, 213, 190, 247), 1500),      # ①左欄「回應設定」
        (oam, 0, (548, 276, 600, 312), 2000),     # ②打開「聊天」
        (oam, 0, (548, 452, 600, 488), 2000),     # ③打開「Webhook」
        (oam, 200, None, 130),
        (oam, 420, None, 130),
        (oam, 610, None, 300),
        (oam, 610, (546, 750, 652, 792), 2400),   # ④聊天的回應方式
    ], x0=0, x1=1352)
