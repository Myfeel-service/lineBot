  ...
  >
      ...
      <div class="lp-wrap lp-hero__center">
        ...
        <div class="lp-rev" :class="{ 'is-open': rev.tally }">
          <div>
            ...
            <!-- ⚠️ 2026-09-10 老闆「把『位』縮小」：它是**單位**不是數字，跟 580 同一個字級
                 會跟數字搶。⛔ 但仍然要留在**同一行**（09-09 拍板，理由見上面①），所以是
                 包一個 span 縮字級、不是換行。⚠️ 空格放在 span 裡面＝跟著「位」縮小
                 （留在外面就是大字級的空格，會在數字與單位之間開一個洞）。
                 ⚠️ `.lp-tally b` 的 textContent 仍然要是「580 位」——驗收工具靠它抓
                 「位有沒有掉出大字行」，⛔別把空格拿掉或改成 &nbsp;。 -->
            <p class="lp-tally"><b>{{ tallyShown }}<span class="lp-tally__unit"> 位</span></b>快忘記你的客人，被這一則推播叫醒</p>
          </div>
        </div>
    ...
    <!-- ── 為什麼卡住：「沒有的那夜 vs 有的同一晚」對照舞台（09-09 第三版，使用者拍板）──
         ...
         ⚠️ 整場戲仍**全自動**（拍板「不要讓客人按」）：捲到 76% 線演一次（第四輪從 7.6s
            ...
            泡泡的副標浮出。底案＝演完的完整狀態（減少動態／沒 JS 直接看到）。
         ⚠️ 文案紅線：四關標題／副標／答案沿用拍板文案一字不動；零成效數字（23:41／11:06
            是情境時間）。「示意情境，以咖啡店為例」標示 09-09 第六輪由使用者拍板移除——
            成立的前提是**對話裡沒有任何數字宣稱**且店名「◯◯咖啡」本身就是明白的占位符；
            ⛔ 未來若把成效數字寫進這段對話，示意標示要一起加回來（08-26 誠實機制）。 -->
        ...
        <div class="lp-turn lp-reveal">
          ...
          <div class="lp-bubble">
            <!-- 09-06 老闆給的改寫原文照用（原「這件事，老闆一直卡在這四關」被回饋難懂）。
                 ⚠️ 這句副標掛 .lp-bubble__relief＝下面的對照舞台**演完**才淡入（樣式在
                    _landing.scss「對照舞台的戲」）：不然痛還沒演完就先被這句爆雷「都有解」。
                    ⛔ 兩句都是老闆原文、字一個都不能動，動的只有出現時機；減少動態／沒 JS／
                    不支援 :has() 的瀏覽器從第一眼就看得到它（藏它的規則失效＝顯示，fail-open）。
                 ⚠️ 2026-09-10 老闆**自己翻案**改了副標：原句「還好，每一關都有 MiniMe 能接住的
                    解法——」→ 現在這句。09-09 那條「字一個都不能動」講的是那時的原文，
                    ⛔ 別因為看到舊註解就把上一句改回去。 -->
            <h2>你知道 LINE 會員經營、客服溝通<br>有多重要，但你總是<span class="mark">卡在這四關</span>。</h2>
            <p class="lp-bubble__relief">讓 {{ brandName }} 為你解決這所有的問題。</p>
          </div>
        </div>
            ...
            <div class="lp-rails__flow">
              ...
              <!-- ── 圖文選單：一條窄帶（2026-08-27 老闆拍板從 791px 的大卡降級）──
                   ...
                   ⚠️ 資訊層次刻意分三層：標題＝客人得到什麼／一句話＝為什麼省事／
                      ...
                      盒子空，圖示列把留白撐起來也讓整區語彙一致）；當時文案沿用原句、
                      只收掉「想更省事，」這個連接詞。⚠️ **兩列的字 2026-09-10 換過**
                      （老闆的Ｂ版提案，理由寫在下面 h3 上面那塊）。
              ...
              <div id="cap-richmenu" class="lp-band lp-pane lp-reveal">
                <div class="lp-band__text">
                  ...
                  <!-- ⚠️ 2026-09-10 老闆給了兩版提案要我挑（Ａ「把 LINE 從聊天工具，變成你的
                       品牌入口」／Ｂ「把常用資訊放進 LINE，客人自己找得到」），拍板走 **Ｂ 的語言**：
                       Ａ 的「品牌入口」是行銷術語，這頁的規矩是反術語白話（老闆自己的在乎點）；
                       Ｂ 講的是店家真的會遇到的事——客人在問、你在回。
                       ⚠️ **標題仍然只有「圖文選單」四個字**（09-08 拍板沒有被推翻）：提案的第一句
                          是**賣點**、不是標題，它的位置在下面這段 p。理由三條：①左軌第四條的
                          標籤就是「圖文選單」，標題換成一句長話，軌與內容會變成兩套說法
                          ②「圖文選單」是 LINE 官方用語，開過官方帳號的店家本來就認得
                          ③這顆 h3 一長，1101px 那一級的色塊（334px）就折兩行——級距是回推它算的
                          （見 _landing.scss 斷點那段）。⛔ 要把長標題搬上來請先問老闆，順手改
                          會同時撞掉這三件事。 -->
                  <h3>圖文選單</h3>
                  <p>商品、活動、預約、門市、找客服，都能放進 LINE 下方的選單——客人一打開就自己點得到，不用一直問，你也不用一直回。</p>
                  <div class="lp-app">
                  ...
                  <div class="lp-app">
                    ...
                    <div><b>之後還能直接跟 {{ brandName }} 說：「幫我改選單」<span class="lp-soon lp-soon--inline">即將推出</span></b></div>
                  </div>
    ...
    <section class="lp-section lp-section--tint">
      <div class="lp-wrap">
        <div class="lp-turn lp-reveal">
          <span class="lp-turn__ava"><BrandLogo mark on-color alt="" /></span>
          <div class="lp-bubble">
            <!-- ⚠️ 2026-09-10 老闆改的原文：「開店的人」→「其他老闆」（螢光筆跟著換詞，
                 ⛔ 別把 .mark 留在舊字上——這頁全部的螢光筆都只畫在「換誰來說」那個詞上）。 -->
            <h2>別只聽我說，<br>聽聽<span class="mark">其他老闆</span>怎麼說。</h2>
          </div>
        </div>
      </div>
