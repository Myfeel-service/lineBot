  ...
  >
    ...
    <section id="pricing" class="lp-section" :class="tintWhenFastShown">
      <div class="lp-wrap">
        ...
        <!-- 「AI 知識量」那半句的白話解釋是 09-06 老闆回饋補的（「知識庫兩份資料不好理解」）：
             方案卡上只有幾個字，第一次看的人不知道那是什麼、多算多。
             09-07（`D-69`）單位從「幾份資料」改成「幾條」，解釋句跟著換：份數會被
             「十份併成一個檔案」繞過，而且客人真正想知道的是「AI 記得住多少東西」。
             ⚠️ 2026-09-08（使用者「方案卡下面的小字是否可以減少」）103 字 → 67 字、
                桌機 3 行 → 2 行（手機 4 → 3）。砍掉的是**卡上已經印過的東西**：
                「每月 AI 回覆則數、團隊席次」三列就寫在正上方四張卡上，用文字再唸一次
                跟 09-04 砍掉的「每個帳號都有免費額度」是同一種重複（那句在重講第一張卡）。
                「差別只在規模」照留＝那三列要怎麼讀的鑰匙；例子從兩個（一題問答、一個價目）
                收成一個換算（一份菜單約 30～60 條）——抽象定義留一句就夠，客人真正要的是
                「我手上這份東西大概算幾條」。 -->
        <p class="lp-plans__note lp-reveal">
          <b>AI 客服四個方案都能用</b>，差別只在<b>規模</b>。「AI 知識量」的一條<span class="lp-nb">＝AI 記住的一件事</span>，一份菜單大約 <span class="lp-nb">30～60 條</span>；隨時可以升級或降級。
        </p>
    ...
    <section class="lp-section lp-cta">
      ...
      <div class="lp-wrap lp-cta__in">
        ...
        <!-- ⚠️ 口徑（2026-09-07）：按鈕寫「免費打造」、小字卻只講付費價＝在成交的那一刻
             ...
             ⚠️ 2026-09-08（使用者「這塊的文字是否也是可以減少」）48 字 → 28 字：只留**整頁
                只有這裡講**的兩件事＝免費不用綁卡、取消後服務用到本期結束。
                ⛔ 這兩件都不能再砍：前者是「免費打造」那顆按鈕的真假，後者是「取消不等於
                   當天斷線、也不等於退錢」，整站只有 /product-info 與 /refund 另有全文。
                砍掉的是重複：①價格——大字報價講過，而且捲到這一區時**黏性行動條**就在螢幕
                底部寫著「一個月只要 NT$399 起」（實測這個位置它是升起的），同一個數字同時
                看得到兩次；②「不綁約」——「隨時可取消」已經是同一個承諾，而完整句在大字
                底下與行動條各留了一次。 -->
        <p class="lp-cta__fine">免費方案不用綁卡 · 付費方案隨時可取消，<span class="lp-nb">服務用到本期結束</span></p>
      </div>
    </section>
