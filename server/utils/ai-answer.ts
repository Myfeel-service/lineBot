...
// 「兩台的按鍵鎖操作方式相同」(實測 R1)。
// ⚠️ 除濕量/坪數/水箱容量是家電字眼(從實際租戶案例長出來,2026-08-11 盤點時定調):
// 別的行業不命中只是「排序不變」,不會答錯——這裡只重排、不改分數、不刪內容,
// 所以可以接受。⛔ 給後人的線:這類「內容理解」清單只能做 fail-soft 的排序偏好,
// 不能拿去做會改資料或擋行為的判斷;那種要用通用原則交給 LLM(見 C-17 的教訓)。
  ...
  const userPrompt = [
    ...
    ...(!isCompare && !isList
      ? [
          ...
          '若客人**有指名**某類產品（例：空氣清淨機），但下方卡片全是**別的產品**：不要拿其他產品的做法叫客人「參考」——那會誤導（例：叫清淨機客人去排空除濕機水箱）。這種情況一律 hasInfo=false、answer 留空。',
        ]
      : []),
    ...
    // 2026-08-11 通用化：原本整段寫「募資金額／集資總額」，是行業字眼進了所有租戶共用的
    // prompt（老闆拍板：內容規則不綁特定行業）。原則本身跨行業成立——任何「整個活動的
    // 累計數字」被拿去回單價都是同一種錯（補習班的「累積 3000 名學員」不是課程價格）。
    '  - **「整個活動／專案的累計數字」不是單一商品的售價**：知識卡裡的活動總金額、累計參與人數、達成率是整個活動加總的成績，不是一件商品的價錢，拿來回答價格客人會誤會。問價格時**一個字都不要提到這些數字**；沒有售價資料就只回「目前沒有標示售價」並附上官網連結。',
